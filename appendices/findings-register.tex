...
file, with no library mechanism to detect or recover. & Safety & I2--I3/E3 &
...
\code{LONG-SAF-001} &
An archived file becomes unreadable or uninterpretable because a required
filter, VFD, or VOL connector is no longer available. & Safety & I2--I3/E2 &
Moderate--High & Provisional & Low \\
...
reference implementation, so conformance is not determinable. & Safety &
I2--I3/E2--E3 & Moderate--High & Provisional & Moderate \\
...
Table~\ref{tab:findings-ssp-crosswalk} relates each record above to the SSP SIG
threat, hazard, and privacy registries. Three cautions govern its use, and all
...
from pathological chunk layouts. The promoted assert-only instance sits under
\code{ATK-010}. \\
...
declines to rate pending \code{RM-2D} measurement. See
Table~\ref{tab:haz-crosswalk}. \\
...
\code{LONG-SAF-001} &
\code{HAZ-014} (partial) &
The registry covers missing filter plugins. Neither registry covers VFD or VOL
archival dependency; that scope originates here. \\
...
\textbf{I1} parser/spec divergence, but neither registry rates defects in the
normative artifact itself. \\
...
\textbf{One instance has been promoted} into \code{CORE-SEC-002} on reported
reproduction: a v2 B-tree \code{record\_size} of zero reaching an integer
division, \code{SIGFPE} on a release build~\cite{h5policy-case-v2btree-recordsize}.
...
needs a documented negative disposition. Begin from the promoted instance's own
sibling audit, which names further unaudited assertions in the same B-tree
header initializer and in fractal-heap initialization. \\
...
\code{FMT-SAF-001}, \code{APP-SEC-002}, \code{APP-SEC-003},
\code{APP-SEC-005} &
...
Planned and Phase~3 horizons. For \code{FMT-SAF-001} the Near-term obligation
is to \emph{adjudicate the already-identified disputes} --- rule on each, record
the ruling, and change whichever artifact was wrong --- not to build the
machine-readable specification, which is the Phase~3 programme in
\code{FMT-S1}. \\
...
\code{LONG-SAF-001} (by recorded departure) &
...
\code{LONG-SAF-001} routes here rather than at its Moderate--High upper bound;
the departure and its rationale are recorded below. \\
...
Items \code{CORE-SEC-003}, \code{EXT-PRIV-001}, and \code{SUP-SAFE-001}
...
longer does. The change is explained in its record below: the evidence gap was
misidentified as an absence of knowledge about the library's behaviour when it
is in fact a documented absence of a mechanism, which bounds the exposure axis
rather than leaving it open.

\paragraph{Recorded departure from the default routing map for
\code{LONG-SAF-001}.}
Section~\ref{sec:risk-rubric} maps a Moderate--High range, routed at its upper
bound, to \emph{Near-term}. \code{LONG-SAF-001} is routed \emph{Planned}
instead. Rationale: the finding states a decadal assessment horizon, and
decision rule 7 forbids directly ordering ratings that use materially different
horizons against those that do not. The harm accrues gradually, is not acute in
any identified present deployment, and is prevented by decisions taken at write
and archive time rather than by an urgent change to a running system. A 30-day
treatment clock would therefore measure nothing. This departure requires an
accountable decision at adoption; if an archive is identified whose readers are
already failing, that scoped instance is expedited and rated without a horizon
qualifier.
...
\noindent\textbf{Confirmed instance promoted from \code{CORE-SEC-003}.}
One assert-masked deserializer invariant has been carried to the standard that
Section~\ref{sec:risk-rubric} requires for a rated scenario, and is recorded
here rather than left in the evidence backlog. A version-2 B-tree header
declaring \code{record\_size == 0} reaches an integer division while the leaf
record capacity is derived; the only guard is
\code{assert(cparam->rrec\_size > 0)} in \code{H5B2hdr.c}, which is absent from
release builds compiled with \code{-DNDEBUG}. The reported measurement records
\code{SIGFPE} on a shipped \code{h5dump -H} against HDF5 2.3.0, on the public
API path \code{H5Fopen} $\rightarrow$ \code{H5Gopen2} $\rightarrow$
\code{H5Gget\_info}, on both x86-64 and i386 builds, with the condition
re-checked against a later source state~\cite{h5policy-case-v2btree-recordsize}.

This satisfies the three conditions \code{CORE-SEC-003} had left open for its
population: the assertion is the sole guard, it is release-reachable in a
supported version, and the adverse outcome is observed rather than inferred.
Under \code{RM-1E}, a confirmed adverse path is routed by its consequence
without waiting for the catalog; the consequence here is controlled-process
termination, which is this record's scenario rather than
\code{CORE-SEC-001}'s. The instance is therefore promoted into
\code{CORE-SEC-002}.

Two qualifications are part of the record. The measurement is
\emph{source-reported}: this assessment did not independently reproduce it, and
the reporting project states the affected version range as \emph{not
established}, having measured one release without version archaeology. Second,
a related variant --- a leaf capacity of exactly zero rather than a division by
zero --- is reported to produce intermittent heap corruption under an older
h5py-linked library, which if characterized would belong to
\code{CORE-SEC-001} instead. That variant is expressly recorded by its
reporters as observed but not characterized, and this report does not rate it.
...
The promoted instance does not change the tier: a confirmed termination path is
the I2 consequence already assigned. It does not raise confidence to High
either, because the affected version boundary --- a material rating assumption
under Section~\ref{sec:risk-rubric} --- remains unestablished. What it does
change is the \emph{status of the mechanism}: release-build absence of a
file-derived check is no longer a hypothesis for this record, and the
\code{RM-1B} negative test for it is a concrete, available deliverable rather
than one contingent on the \code{RM-1E} catalog.
...
\code{ATK-009}; the promoted instance also corresponds to \code{ATK-010}.
...
flush, or close sequence. The write fails with \code{ENOSPC}. The library
provides no mechanism by which the application can be notified at the point of
failure, no defined recovery procedure, and no way to mark the resulting file
as suspect. The file is left incomplete or internally inconsistent, and data
that has not been independently preserved may be lost.

\noindent\textbf{Scope and evidence.}
The assessed population is any deployment that writes HDF5 files to storage
which can reach capacity --- which is not a narrow population. The controlling
evidence is architectural rather than statistical, and it is a maintainer
statement in the cited design discussion: \emph{there is no HDF5 story for}
\code{ENOSPC}, \emph{and no callback for the condition that the virtual file
driver has just encountered it}. The same discussion records that the current
practice is a workaround --- configuring the metadata cache to discard on
evict --- rather than a supported control, and enumerates four candidate
treatments: space reservation via \code{posix\_fallocate}, an explicit
poisoned-file state, a pre-flush capacity check, and a virtual-file-driver
error callback that lets the caller choose to retry, fail fast, or
panic~\cite{forum-enospc-11714}. The condition also appears in general
file-corruption reports across the community
forum~\cite{forum-recover-1146,forum-recover-5441}.

\noindent\textbf{Why this is no longer NE.}
An earlier revision recorded this finding as NE on the ground that the recovery
path was not bounded. That was a category error, and correcting it is the
substantive change in this record. The recovery path \emph{is} bounded: it does
not exist. That is a verifiable property of the published API surface, not an
unmeasured quantity. It also determines the exposure axis, because a control
that is absent cannot intermittently succeed --- every occurrence of the
triggering condition during a write reaches the unsafe state. Exposure
therefore reduces to how often storage fills during a write, which is an
ordinary operational fact rather than an open question about HDF5. Treating the
documented absence of a control as an absence of evidence understated this
finding.
...
records is credible but is not established by the cited evidence, and decision
rule 1 requires the greatest \emph{supported} impact rather than a worst case.
E3 reflects a common workflow with few material preconditions. The rating is
\textbf{I2--I3/E3, Moderate--High, Provisional, Low confidence}. Confidence is
Low because no failure sequence, durable-state result, recoverability outcome,
or population has been measured; \code{RM-2D} remains required and would move
confidence rather than the tier. Current residual risk was not evaluated.
...
\subsection{Format and Specification Findings}

\subsubsection*{FMT-SAF-001 --- Conformance is not determinable}

\noindent\textbf{Scenario.}
An independent implementation writes or reads an HDF5 structure according to
the published file-format specification. The reference library behaves
differently. Because the specification and the reference implementation
disagree --- and because no procedure exists to rule on which is authoritative
--- neither party can establish whether the file conforms. Files are rejected,
silently reinterpreted, or accepted by one implementation and not another, and
the dispute is resolved by whoever concedes rather than by reference to a
normative artifact.

\noindent\textbf{Scope and evidence.}
Two instances are documented, and they fail in different directions, which is
what makes the class rather than either case the finding.

\begin{description}[leftmargin=1.5em,style=nextline]
    \item[The library was non-conforming.]
    Issue \#6409. The format required an encoded chunk-dimension width
    sufficient to hold the value and did not require the smallest possible
    width; HDF5 2.0.0, 2.1.0, and 2.1.1 rejected a sufficient but nonminimal
    encoding anyway. The specification was correct and the reference reader was
    wrong, and the reader was changed~\cite{hdf5-gh-issue-6409,hdf5-gh-pr-6432}.

    \item[Which artifact is wrong is undetermined.]
    Issue \#6616. The specification documents the group info message as
    optional; the C library raises a ``message type not found'' error when it
    is absent, so it cannot modify a file written without one. The reporter
    states the two possible resolutions explicitly --- the library should fall
    back to default group-info values, or the specification should be corrected
    to say the message is required --- and at the evidence cutoff the issue is
    open, labelled a documentation bug, assigned, and milestoned, with no
    ruling recorded~\cite{hdf5-gh-issue-6616}.
\end{description}

Both were discovered the same way: an independent implementation exercised a
construct and diverged from the reference library. Neither was found by
inspecting the specification, and neither would have been visible at all if
libhdf5 were the only implementation, because a sole implementation's behaviour
is indistinguishable from the format.

\noindent\textbf{Why this is a distinct finding.}
\code{IND-SAF-001} and \code{IND-SAF-002} rate the \emph{consequences} of two
particular divergences --- data that became unreadable after a reader upgrade.
This record rates the \emph{condition that produces them}: a normative artifact
that cannot settle the question. It is registered separately because its
treatment is different in kind. Repairing a file or fixing a reader does not
make the next ambiguity determinable, and the report's own method assumes
determinability: Section~\ref{sec:assessment-methodology} states that where
implementations disagree the analysis distinguishes the normative requirement
from writer behaviour and reader validation. Issue \#6616 is a case where that
distinction cannot currently be drawn, which is a limitation of the method as
well as a finding about the format.

\noindent\textbf{Rating basis.}
I2 reflects a bounded, repairable interoperability or data-access failure ---
the evidenced outcome in both instances. I3 reflects consequential loss where
an archive or a scientific result depends on the contested interpretation and
the dispute is not resolved in time; this is credible but not evidenced, so the
range is stated rather than the upper bound asserted. E2 reflects the
preconditions for any single ambiguity: a construct must be exercised, an
implementation must diverge, and files must cross implementations. E3 reflects
that the condition now recurs --- two instances surfaced within months of each
other in 2026 --- and that exposure rises with the number of independent
implementations, since each ambiguity is resolved independently by every one of
them. The rating is \textbf{I2--I3/E2--E3, Moderate--High, Provisional,
Moderate confidence}. Confidence is Moderate because both instances rest on
primary evidence but no systematic survey of the specification has been
performed; the extent of the defect population is unmeasured.

\noindent\textbf{A second-order effect worth recording.}
In \#6409 the independent writer changed its output to match the reference
reader, even though its original output was specification-permitted. That is
the rational local decision and the corrosive global one: under-specification
makes implementations converge on the reference library's behaviour rather than
on the specification, so the specification loses normativity exactly as the
number of implementations that should be reinforcing it grows. A dispute
resolved by concession leaves the ambiguity in place for the next
implementation to rediscover.

\noindent\textbf{Response.}
Routed Near-term at the upper bound. The Near-term obligation is
\emph{adjudication of the identified disputes}: for each open case, rule which
artifact is authoritative, record the ruling and its rationale, and change the
losing artifact. \#6616 is the immediate instance and already has an owner and
a milestone. The systematic treatment --- a machine-readable normative
specification and a standing procedure for future disputes --- is
\code{FMT-S1} and runs on the Phase~3 horizon. Adjudicating two open cases does
not require the programme, and the programme does not excuse leaving them open.

\noindent\textbf{Linked recommendations.}
\code{FMT-S1}; \code{IND-S1} for the contested-reading fixtures; \code{CORE-S1},
whose invariant registry must record which invariants are
specification-derived and which are reference-implementation-derived, because
the two carry different authority.

\noindent\textbf{SSP SIG correspondence.}
No direct counterpart. The hazard model has a parser-ambiguity hazard family
(\code{H6}) and the independent-implementation model has \textbf{I1}
parser/spec divergence, but neither registry contains an entry for defects in
the normative artifact itself. This scope originates here.

\subsection{Long-Term Accessibility Findings}

\subsubsection*{LONG-SAF-001 --- Extension-dependent loss of interpretability}

\noindent\textbf{Scenario.}
An HDF5 file is retained in an archive. Reading or interpreting it requires a
filter plugin, virtual file driver, or VOL connector that is not part of the
core library. At some later point that extension is unavailable for the
reader's platform, unbuildable against a current toolchain, unmaintained, or
simply unidentifiable from the file itself. The bytes survive; the data does
not. Depending on the extension class, the outcome ranges from a dataset that
cannot be decompressed to a file whose contents were never fully self-contained
in the first place.

\noindent\textbf{Assessment horizon.}
This finding states an assessment horizon of \textbf{ten years or more} from
the date of writing. The horizon is material: at a one-year horizon the
scenario is largely hypothetical, and at a multi-decade horizon it approaches
certainty for any file depending on a single-maintainer extension. Under
decision rule 7 of Section~\ref{sec:risk-rubric}, this rating must not be
directly ordered against ratings that state no horizon.

\noindent\textbf{Scope and evidence.}
The assessed population is archived files that depend on a non-core extension
for read or interpretation. The immediate mechanism is already observable at
short horizons: community reports record users unable to read data because a
required compression filter was absent from their build or platform, across a
fifteen-year span and several filter families
...
The three extension classes are not equally exposed, and the report does not
claim they are. A missing \emph{filter} is the best-evidenced and most bounded
case: the file identifies the filter by numeric identifier, so the dependency
is at least discoverable, and re-implementation is possible in principle. A
missing \emph{VFD} affects whether the bytes can be reached at all. A missing
\emph{VOL connector} is the most severe and the least evidenced, because the
data may never have been in an HDF5 file in the first place --- the connector
may have been mapping to an entirely different store, in which case
``preserving the HDF5 file'' preserves nothing.

\noindent\textbf{Rating basis.}
I2 reflects a bounded interoperability failure where the extension can be
recovered, rebuilt, or re-implemented and the data is then readable. I3
reflects consequential loss where it cannot and the records are unique or
scientifically significant. E2 reflects the preconditions --- the file must
depend on a non-core extension, and the extension must have become unavailable
within the horizon --- which is realistic but not universal, since a large
share of HDF5 data uses only core features or \code{gzip}. The rating is
\textbf{I2--I3/E2, Moderate--High, Provisional, Low confidence}, at the stated
horizon.

\noindent\textbf{Response.}
Routed \emph{Planned} by recorded departure from the default map; see the
rationale in Section~\ref{sec:findings-evidence-actions}. The controlling point
is that the effective treatment window is at \emph{write and deposit} time, not
at read time: once an archive holds files whose extension is gone, the options
are recovery archaeology or loss. Treatment is therefore a producer-side and
curator-side practice, delivered through \code{LONG-S1}.
...
\noindent\textbf{SSP SIG correspondence.}
\code{HAZ-014} covers the filter-plugin portability slice at Moderate.
\textbf{Neither registry covers VFD or VOL archival dependency}, and this
report contributes that scope.

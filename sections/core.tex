...
~\cite{h5lens-issue-87}. A fixed August 25 develop snapshot corroborates
...
\subsubsection{Prior Art and the Remaining Work}
\label{sec:core-prior-art}

H5Lens / \code{h5lens} already provides machine-readable format
definitions, \code{h5policy} preflight profiles and a regression corpus, a
versioned invariant registry, the evidence-gated \code{h5patch} repair planner,
and differential measurements against selected libhdf5 builds
\cite{h5lens,h5policy,h5policy-invariant-registry,h5patch}. Its strategy
and bounded-decoder work also define the decode--validate--construct
architecture~\cite{hdf5-cve-strategy,hdf5-bounded-raw-decode}. Two refinements
are material: validation occurs at record-local, aggregate-object, and
reference-graph scope; and materialization must remain separate from activation
(cache publication, ID registration, plugin loading, decompression, callbacks,
and external-file access). Locally valid metadata can fail only at a wider
scope, and successful parsing must not authorize a side effect. The latter is
the core-library counterpart of \code{APP-SEC-001}.

Table~\ref{tab:core-prior-art-rescope} distinguishes reuse from remaining
implementation. Effort is separate from inherent risk and is subject to the
licensing boundary in Section~\ref{sec:roadmap-licensing}: identifiers,
boundaries, fixtures, and measurements may be usable after provenance, licence,
and, where needed, legal review; GPLv3 implementation source is not directly
consumable into the BSD-licensed library.
...
\setlength{\LTleft}{-0.9in}
\setlength{\LTright}{-0.9in}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.10\widetablewidth}>{\raggedright\arraybackslash}p{0.35\widetablewidth}>{\raggedright\arraybackslash}p{0.41\widetablewidth}>{\raggedright\arraybackslash}p{0.12\widetablewidth}@{}}
\caption{Recommendations re-scoped against existing H5Lens work}
\label{tab:core-prior-art-rescope}\\
\hline
\textbf{Rec.} & \textbf{Existing asset} & \textbf{Remaining work} & \textbf{Effort} \\
\hline
\endfirsthead
...
\hline
\endlastfoot
\code{CORE-S1} & Versioned per-family invariant registry with finding, test,
fuzz, enforcement, and migration mappings. & Adopt the vocabulary; implement
always-active enforcement inside libhdf5 and prove debug/release parity. &
\textbf{Medium} \\
\code{CORE-S2} & Three validation scopes, materialization/activation rule,
seven-phase migration design, and bounded-decoder demonstration. & Migrate the
library; design reuse does not reduce that work. & \textbf{Large} \\
\code{CORE-S4} & Four exercised profiles with feature policy and resource
budgets. & Port semantics to the \code{H5P} API and enforce defaults and policy
inside libhdf5. & \textbf{Medium} \\
\code{CORE-S5} & Per-family mutation recipes, truncation sweeps, and a
boundary-value coverage inventory. & Add OSS-Fuzz integration and the missing
per-family targets. & \textbf{Medium} \\
\code{CORE-S6} & Manifested malformed-input and valid-control corpus with
per-fixture outcomes. & Review specimen provenance; integrate HDF5 CI and
debug/\code{-DNDEBUG} parity. & \textbf{Small} \\
\code{APP-S2} & Working profile-based preflight validator with structured
results. & Bind the verdict to the same handle or verified identity used at
load time; otherwise preflight remains advisory. & \textbf{Medium} \\
\end{longtable}
\endgroup

H5Lens does not supply this report's application-boundary findings,
independent-implementation analysis, inherent/residual-risk separation,
response clocks, accountable owners, or ecosystem attribution; those are
complementary governance contributions. One loop also remains open: many
tracked divergences are reported as oracle-rejected but libhdf5-unfixed. Two
concern external-file-list handling, including an uninvestigated case in which
an external file is opened before its wrapped segment is validated
\cite{h5policy-case-efl-open}. If substantiated, it is an
activation-before-validation instance relevant to \code{APP-SEC-001}; this
report did not evaluate it. \code{RM-1E} therefore takes the full
oracle-hardened set as triage input.

\subsubsection{Core Security Control Program}

The objective is controlled rejection of any hostile byte stream: no crash,
assertion, leak, undefined behaviour, unbounded work, or late encode/flush
failure caused by an invalid decode. The authoritative ownership, finding
mapping, effort, package, and acceptance evidence for \code{CORE-S0} through
\code{CORE-S12} are in Appendix~\ref{app:recommendation-register} and
Appendix~\ref{app:phased-roadmap}. The synthesis below preserves the control
logic without duplicating repository layouts, sample APIs, YAML, or checklists.
...
\setlength{\LTleft}{-0.9in}
\setlength{\LTright}{-0.9in}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.11\widetablewidth}>{\raggedright\arraybackslash}p{0.23\widetablewidth}>{\raggedright\arraybackslash}p{0.45\widetablewidth}>{\raggedright\arraybackslash}p{0.17\widetablewidth}@{}}
\caption{Core security recommendations: strategic synthesis}
\label{tab:core-security-synthesis}\\
\hline
\textbf{ID} & \textbf{Control} & \textbf{Required outcome} & \textbf{Roadmap} \\
\hline
\endfirsthead
\caption[]{Core security recommendations: strategic synthesis, continued}\\
\hline
\textbf{ID} & \textbf{Control} & \textbf{Required outcome} & \textbf{Roadmap} \\
\hline
\endhead
...
\hline
\endlastfoot
\code{CORE-S0} & Separate defect and class closure. & A local repair may close
the affected defect, including under embargo, but the paired class-prevention
record stays open until sibling paths are structurally covered. & \code{RM-1B},
\code{RM-3C}, \code{RM-4A} \\
\code{CORE-S1} & Adopt the invariant registry. & Each declared on-disk
condition maps to an always-active libhdf5 check, expected error, test, fuzz
target, and migration state. Issue~\#87's seven families are the first audit
perimeter; consequence and supported-release reachability, not candidate count,
set tranche order. & \code{RM-3A} \\
\code{CORE-S2} & Enforce the parser boundary. & Bounded decode precedes
record-local, object-wide, and graph validation; construction precedes a
separately authorized activation step. No raw file quantity becomes a trusted
pointer or side effect. & \code{RM-3B} \\
\code{CORE-S3} & Centralize checked arithmetic. & File-derived addition,
multiplication, address ranges, allocation, copying, cursor movement, and graph
traversal use checked wrappers or recorded exemptions. & \code{RM-2A},
\code{RM-3A} \\
\code{CORE-S4} & Make open policy explicit. & \code{legacy},
\code{trusted-fast}, \code{untrusted-strict}, and \code{forensic} profiles bind
validation, external access, plugins, traversal, metadata, allocation, and
filter-expansion budgets to tested defaults. & \code{RM-2A} \\
\code{CORE-S5} & Fuzz structures and invariants. & Whole-file, structure-aware,
and single-invariant mutation cover object headers, datatypes, dataspaces,
layouts, heaps, filters, cache images, and tool rendering; coverage is reported
per component. & \code{RM-3C} \\
\code{CORE-S6} & Make regression permanent and cheap. & Every closed case has
a minimized, provenance-reviewed seed, expected controlled outcome, invariant
mapping, and identical assertion-enabled/\code{-DNDEBUG} rejection. &
\code{RM-1B}, \code{RM-2C}, \code{RM-3C} \\
\code{CORE-S7} & Block unsafe parser changes. & Review and automated gates
require explicit bounds, checked arithmetic and copies, bounded traversal,
partial-initialization cleanup, policy-aware plugin use, malformed-input tests,
and always-active counterparts to file-derived assertions. & \code{RM-1E},
\code{RM-3C} \\
\code{CORE-S8} & Separate compatibility from safety. & Valid legacy files
remain readable, but tolerated corruption does not become compatibility and
repair never occurs silently during normal open. & \code{RM-2C}, \code{RM-4A} \\
\code{CORE-S9} & Treat tools as attack surfaces. & Inspection and conversion
tools use forensic or strict profiles; rendering is bounded; legacy converters
are explicit opt-ins; plugin-dependent tools require warning or approval. &
\code{RM-1B}, \code{RM-2A}, \code{RM-3C} \\
\code{CORE-S10} & Govern executable extensions. & A protected manifest and
allowlist bind identity, profile, capability, and resource limits; signatures
prove provenance, not resource safety. & \code{RM-3D} \\
\code{CORE-S11} & Coordinate supply-chain controls. & Independently owned
Scorecard, CodeQL, pinned-action, SBOM, signing, checksum, and SLSA controls run
in parallel with parser work; only concrete dependencies affect sequence. &
\code{RM-3E}, \code{RM-4A} \\
\code{CORE-S12} & Measure class elimination. & A public dashboard reports a
versioned denominator and named, expiring exceptions; metrics are coverage
evidence, not defect closure. & \code{RM-1B}, \code{RM-3C}, \code{RM-4A} \\
\end{longtable}
\endgroup

The defect/class distinction requires the seven artifacts in
Table~\ref{tab:cve-closure-artifacts}. Recent fixes illustrate the intended
direction: the file-space page-size fix converted an assertion into a
corrupted-file error and reused its fuzzer, while cache-image reconstruction
added buffer-size tracking, overflow checks, validation, and safe cleanup
\cite{commit_804f3ba,commit_3914bb7}. The 2024 dataset-storage fix similarly
added multiplication-overflow and file-bound checks~\cite{commit_ce53bc0}.
These local repairs should become programme-wide invariants and primitives.
...
reviewer-checklist update & reintroduction gate \\
\end{longtable}
\endgroup

Structure-aware fuzzing supplements whole-file mutation; OSS-Fuzz supplies
long-running public execution and CI supplies fast feedback. These cadences are
acceptance gates, not risk priorities.
...
\hline
\endfoot
PR & minimized CVE corpus under \code{ASan}/\code{UBSan}/\code{LSan} \\
...
Backward compatibility does not authorize malformed metadata. The profile
...
\hline
\textbf{Case} & \textbf{Required behaviour} \\
\hline
\endfirsthead
\hline
\textbf{Case} & \textbf{Required behaviour} \\
...
ambiguous legacy construct & accept only in \code{legacy}/\code{trusted-fast};
warn in \code{forensic}; reject in \code{untrusted-strict} \\
...
unknown message with unsafe flags & reject unless the profile explicitly
allows preservation \\
\end{longtable}
\endgroup
...
\begin{longtable}{@{}p{0.72\linewidth}p{0.24\linewidth}@{}}
\caption{Security programme metrics}
...
declared invariants enforced with \code{-DNDEBUG} & 100\% \\

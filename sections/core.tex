...
malicious actor, as defined in Section~\ref{sec:assessment-dimensions}. Three
conditions were examined, and none reached the evidence threshold for a rated
finding in this revision.

\begin{description}[leftmargin=1.5em,style=nextline]
    \item[Storage exhaustion during write or close.]
    Community reports describe files left incomplete or corrupted when the
    underlying storage fills during writing or on close, with data loss in
    some accounts~\cite{forum-11714,forum-file-corruption}. The report has not
    bounded the API sequence, version, filesystem and buffering behavior,
    recovery path, uniqueness of the affected data, or frequency, so neither
    axis of the rubric can be applied. This is registered as
    \code{CORE-SAFE-001} with status NE.

    \item[Long-term accessibility and interpretability.]
    Preservation depends on more than the primary bytes: format bounds,
    required filters, external resources, application schema, and the
    continued existence of a reader that implements the relevant
    interpretation. Section~\ref{sec:long-haul} develops this; no archive was
    inventoried and no preservation horizon was tested, so it produces no
    formal finding here.

    \item[Reader/writer disagreement as a safety condition.]
    The two 2026 chunk-layout cases in Section~\ref{sec:indie} are the one
    place where safety evidence \emph{was} sufficient, and they are registered
    as \code{IND-SAF-001} and \code{IND-SAF-002}.
\end{description}

\noindent\textbf{Coverage limitation, stated against an existing hazard
analysis.}
Safety is by a wide margin the least developed dimension in this revision. The
...
finding for one, an unrated evidence record for two, and nothing at all for
thirteen.} That is the honest measure of this chapter's safety coverage. The
...
\textbf{Not assessed.} The interrupted-metadata-update scenario is adjacent to
\code{CORE-SAFE-001} but distinct from it: the trigger is process or host
failure rather than storage exhaustion. \code{RM-2D} should widen to cover it. \\
...
\textbf{Registered but unrated.} \code{CORE-SAFE-001}, NE, pending
\code{RM-2D}. The registry supplies the accident chain and three safety
constraints; \code{RM-2D} should adopt those constraints as its hypotheses
rather than starting from the forum reports. \\

\code{HAZ-006} & False sense of durability from flush & 12, High &
\textbf{Not assessed.} Durability signaling was not examined. \\
...
\textbf{Not assessed}, and this is a notable omission given that
\code{IND-SAF-002} and \code{RM-2C} both require a deterministic repair path.
The evidence-gated repair planner in H5Lens is relevant prior art
(Section~\ref{sec:core-prior-art}). \\
...
\textbf{Partially covered as an unrated record.} Within the
\code{SUP-SAFE-001} mismatch theme and the long-term-access discussion in
Section~\ref{sec:long-haul}; no scenario developed. \\
...
--- and could be studied under one widened package at little additional cost.
None of these observations establishes response urgency, and none converts an
unrated record into a rated one.
...
One member of that population has since been carried to reproduction, and it
is worth stating precisely because it bounds what the rest of the record does
and does not claim. A version-2 B-tree header declaring a zero record size
reaches an integer division while the leaf record capacity is derived; the sole
guard is an assertion in \code{H5B2hdr.c}, absent under \code{-DNDEBUG}. The
reported measurement records \code{SIGFPE} from a shipped \code{h5dump -H}
against HDF5 2.3.0 and from the public \code{H5Fopen}/\code{H5Gopen2}/
\code{H5Gget\_info} path, at both 32- and 64-bit
widths~\cite{h5policy-case-v2btree-recordsize}. That instance meets all three
conditions the record leaves open, so it is promoted into \code{CORE-SEC-002}
by its consequence; see Appendix~\ref{app:findings-register}. It does not
establish the size of the remaining population, the affected version range ---
which its reporters expressly decline to assert --- or that other candidates
share its consequence class. A second candidate, the cache-image LRU rank
relation, is reported without a measured release-build
consequence~\cite{h5policy-case-mdci-lru}. One confirmed instance is a reason
to complete the catalog, not a substitute for it.
...
before a tier or urgency can be assigned. The \code{CORE-SEC-003} catalog and

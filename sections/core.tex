...
Four conditions are registered as formal findings in this revision. The first
three are write-side and lifecycle risks and were the least developed part of
...
\begin{description}[leftmargin=1.5em,style=nextline]
    ...
    Storage fills during a write, flush, or close; the write fails with
    \code{ENOSPC}; and the library offers no notification, no recovery
    procedure, and no way to mark the file suspect. The controlling evidence is
    a maintainer statement that there is no HDF5 story for the condition and no
    callback for it, together with four candidate treatments and a documented
    cache workaround used in its place~\cite{forum-enospc-11714}. Registered as
    ...
    fact bounded --- it does not exist --- and that fact constrains exposure
    ...
    of any kind in an earlier revision, which was the largest single omission
    in this chapter.

    \item[Extension-dependent loss of interpretability.]
    An archived file requires a filter, VFD, or VOL connector that is later
    unavailable. Registered as \code{LONG-SAF-001}, \textbf{Moderate--High} at
    ...
    \code{IND-SAF-001} and \code{IND-SAF-002}. These are the short-horizon,
    fully evidenced instance of the same family as \code{LONG-SAF-001}: a file
    that a reader can no longer interpret.
\end{description}
...
\textbf{Covered, and extended.} \code{LONG-SAF-001} rates the archival
scenario at a stated decadal horizon and extends it to VFD and VOL dependency,
which this registry does not cover. \\
...
Today an application cannot learn that the virtual file driver hit
\code{ENOSPC}, cannot choose how to respond, and cannot mark the resulting file
as untrustworthy. Four candidate mechanisms have already been proposed in the
public design discussion; they are complementary rather than alternatives, and
they differ sharply in cost~\cite{forum-enospc-11714}.

...
The term is not coined here. It is already a named family in two SSP SIG
models --- \textbf{I9} in the independent-implementation model and \textbf{W8}
in the Python wrapper model --- each defined as users treating HDF5 as inert
data when the layer they are using can in fact trigger complex parsing, codec
execution, external access, or object reconstruction
\cite{hdf5-indie-model,hdf5-python-wrapper-model}. This report reached the same
conclusion from the incident evidence rather than by applying the models, which
is worth recording as internal consistency: two analytic routes within the same
programme converge on the same root cause. The models are separate artifacts,
not independent third-party validation, so their agreement does not raise the
incident evidence's confidence by itself.

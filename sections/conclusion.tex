...
information, caches, logs, or other workflow artifacts. Fifth, and distinct
from the preceding four in kind: a substantial part of HDF5's risk is not about
input at all. Storage exhaustion, abrupt termination, and extension-dependent
loss of interpretability threaten data that no adversary ever touched, and the
library provides materially weaker guarantees against them than against
malformed input.
...
The authoritative register contains fifteen formal findings: seven Rated and
eight Provisional. Their tiers comprise one Critical, four High, six
Moderate--High ranges, and four Moderate. Three additional observations remain
...
controls the concrete scenario; prevention addresses recurrence and may
continue after the scoped finding is reassessed.
...
\code{CORE-SAFE-001}, \code{CORE-SAFE-002}, \code{FMT-SAF-001},
...
recovery procedure, and an independent copy of unregenerable data --- not the
library change. &
...
\code{IND-SAF-002}, \code{LONG-SAF-001} (recorded departure) &
...
\code{CORE-SEC-003}, \code{EXT-PRIV-001}, \code{SUP-SAFE-001} &
...
source-catalog action but no risk tier of its own; its one reproduced instance
is routed under \code{CORE-SEC-002} by that instance's consequence. The core chapter additionally owns two Near-term write-side safety
...
treatments, and the long-term-access chapter owns one Planned, horizon-
qualified archival finding that an earlier revision left unregistered. Extension privacy and supply-chain mismatch themes
...
The perimeter is no longer entirely hypothetical. One instance has been
carried to reproduction and is promoted into \code{CORE-SEC-002}: a version-2
B-tree header declaring a zero record size reaches an integer division whose
only guard is an assertion, producing \code{SIGFPE} on a shipped tool in a
release build, at both 32- and 64-bit
widths~\cite{h5policy-case-v2btree-recordsize}. That single case settles the
question of kind while leaving the question of extent open, and it does so in
the direction the rubric anticipated: the mechanism is real, the consequence
for this instance is termination rather than corruption, and the affected
version range is still unestablished. It is evidence that the 30-day catalog is
warranted, not evidence of its result. The promotion also demonstrates the
routing rule working as designed --- a confirmed path leaves the backlog and
acquires its consequence's tier, without the backlog record itself ever having
been assigned one.
...
treatment; and the assert-masking observation remains an evidence action until
exact paths and consequences are verified. Together they show why the enforcing
actor, affected scope, consequence, and acceptance evidence must be explicit.
